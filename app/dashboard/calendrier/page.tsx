...
export default async function CalendrierPage() {
  ...
  const { data: posts } = await supabase
    .from('calendrier')
    .select('id, contenu, date_programmee, statut, image_url, generation_id')
    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Vos posts Facebook programmés et publiés.</p>
        </div>
      ...
      <CalendrierView
        posts={posts ?? []}
        linkedinConnected={!!cabinet.facebook_connected}
      />
    </div>
  )
}
